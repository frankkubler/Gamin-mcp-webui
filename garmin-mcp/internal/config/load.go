...
func withNetwork(cfg Config, store *viper.Viper) Config {
	...
	out.SMTPHost = store.GetString(keySMTPHost)
	out.SMTPPort = store.GetInt(keySMTPPort)
	out.SMTPUser = store.GetString(keySMTPUser)
	out.SMTPSecretFile = store.GetString(keySMTPSecretFile)
	out.SMTPFrom = store.GetString(keySMTPFrom)
	out.SMTPTo = store.GetStringSlice(keySMTPTo)
	out.SMTPTLS = store.GetString(keySMTPTLS)
	out.DashboardURL = store.GetString(keyDashboardURL)
